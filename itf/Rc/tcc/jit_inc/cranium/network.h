   ...
   //info_print("Weights Read");
    ...
    for (k = 0; k < network->numConnections; k++){
        ...
        for (i = 0; i < con->weights->rows; i++){
            for (j = 0; j < con->weights->cols; j++){
                fgets(buf, 50, fp);
                sscanf(buf, "%f", &con->weights->data[con->weights->cols * i + j]);
                memset(&buf[0], 0, 50);
                //info_print("%f\n", con->weights->data[con->weights->cols * i + j]);
            }
        }
    }
